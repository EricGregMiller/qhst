 ...
 *
 *   $Id: usercp_register.php,v 1.1.1.1 2005/07/21 05:00:49 sstolyarov Exp $
 *
 *
 ***************************************************************************/
...
// ---------------------------------------
// ---------------------------------------
// Get Asirra validation.
// Returns 'Pass' or 'Fail'
//
include 'Domxml45.php';

function AsirraValidate($iAsirraTicket)
{
  $returnVal = "Fail";
  if (!empty($iAsirraTicket))
  {
    //$asirraTicket = $HTTP_POST_VARS['Asirra_Ticket'];
    //echo "<p>Ticket = $asirraTicket</p>\n";
    //http://challenge.asirra.com/cgi/Asirra?action=ValidateTicket&ticket=$asirraTicket
    $asirraServiceUrl = "http://challenge.asirra.com/cgi/Asirra";
    $asirraValidation = $asirraServiceUrl . "?action=ValidateTicket&ticket=" . $iAsirraTicket;
    //echo "<p>Validation = $asirraValidation</p>\n";
    $asirraValidateXml = file_get_contents($asirraValidation);
    //echo "<p>Xml = $asirraValidateXml</p>\n";
    //$asirraValidate = dom.getElementsByTagName("Result")[0].childNodes[0].data
    $query = "/AsirraValidation/Result";
    $domdoc = domxml_open_mem($asirraValidateXml);
    if (preg_match("/^4/i", phpversion()))
      {
	$xpath = xpath_new_context($domdoc);
	$queryResult = $xpath->xpath_eval($query);
	$queryValidateNode = GetNodeListItem($queryResult->nodeset,0);
      }
    else
      {
	//$domdoc = new DOMDocument($asirraValidateXml);
	$xpath = new DOMXPath($domdoc);
	$queryResult = $xpath->evaluate($query);
	$queryValidateNode = GetNodeListItem($queryResult,0);
      }
    //echo "<p>node = $queryValidateNode</p>\n";
    $returnVal = GetNodeValue($queryValidateNode);
    //echo "<p>return = $returnVal</p>\n";
  }
  return $returnVal;
}
//
...
//
if (
	'yes' == $HTTP_POST_VARS['SubmitButtonPressed'] ||
	...
	$mode == 'register' )
{
	...
	$asirra_validate = AsirraValidate($HTTP_POST_VARS['Asirra_Ticket']);
	//$asirra_validate = AsirraValidate($_REQUEST['Asirra_Ticket']);
	...
	if ( (isset($HTTP_POST_VARS['avatargallery']) || isset($HTTP_POST_VARS['submitavatar']) || isset($HTTP_POST_VARS['cancelavatar'])) && 'yes' != $HTTP_POST_VARS['SubmitButtonPressed'] )
	{
...
//
if ( 'yes' == $HTTP_POST_VARS['SubmitButtonPressed'] )
{
...
function HumanCheckComplete(isHuman)
{
	...
	if ('waiting' == gValidationStatus)
	{
		if (isHuman)
		{
			//alert("Cats correctly identified.");
			formElt = document.getElementById("ProfileAddForm");
			if (formElt)
			{
				//alert("Have form");
				formElt.SubmitButtonPressed.value = 'yes';
				formElt.submit();
			}
			returnCode = true;
		}
...
{
	gValidationStatus = 'timed out';
	SetValidatingScreen();
       	alert("Form validation is not currently working. Please try again.");
}
function SetValidatingScreen()
{
	//alert("SetValidatingScreen");
	validatingScreen = document.getElementById("ValidatingScreen");
	if (validatingScreen)
	{
		//alert("Have form");
		...
		if ('waiting' == gValidationStatus)
		{
			validatingScreen.style.visibility="visible";
		}
		else
		{
			validatingScreen.style.visibility="hidden";
		}
	}
}
function SubmitForm()
{
	//alert("SubmitForm.");
	gValidationStatus = 'waiting';
	SetValidatingScreen();
	Asirra_CheckIfHuman(HumanCheckComplete);
	// After 5 sec, time out.
	gTimeout = setTimeout("ValidationTimedOut();",5000);
	//alert("SubmitForm End");
}
function InitializeForm()
{
	...
	{
        	//alert("Have form");
		formElt.SubmitButtonPressed.value = 'no';
	}
}
//InitializeForm();
</script>
...
//$confirm_image = (@extension_loaded('zlib')) ? '<img src="' . append_sid("profile.$phpEx?mode=confirm&amp;id=$confirm_id") . '" alt="" title="" />' : '<img src="' . append_sid("profile.$phpEx?mode=confirm&amp;id=$confirm_id&amp;c=1") . '" alt="" title="" /><img src="' . append_sid("profile.$phpEx?mode=confirm&amp;id=$confirm_id&amp;c=2") . '" alt="" title="" /><img src="' . append_sid("profile.$phpEx?mode=confirm&amp;id=$confirm_id&amp;c=3") . '" alt="" title="" /><img src="' . append_sid("profile.$phpEx?mode=confirm&amp;id=$confirm_id&amp;c=4") . '" alt="" title="" /><img src="' . append_sid("profile.$phpEx?mode=confirm&amp;id=$confirm_id&amp;c=5") . '" alt="" title="" /><img src="' . append_sid("profile.$phpEx?mode=confirm&amp;id=$confirm_id&amp;c=6") . '" alt="" title="" />';
		...
		$confirm_image = $confirm_script;
		$validating_screen = '<div id="ValidatingScreen" style="margin:0 0 0 0;background-color:#006699;color:#FFA34F;font-size:24px;font-weight:bold;position:absolute;z-index:5;visibility:hidden;"><p>Validating...</p></div>';
		$on_click_script = 'onclick="SubmitForm()"';
		$s_hidden_fields .= '<input type="hidden" name="SubmitButtonPressed" value="no"/>';
	...
	$template->assign_vars(array(
		...
		'VALIDATING_SCREEN' => $validating_screen, 
		'YIM' => $yim,
		'ICQ' => $icq,
		'MSN' => $msn,
		'AIM' => $aim,
