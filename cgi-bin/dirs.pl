...
# dirs.pl: A program to do two things. First, show any .gifs, .pngs or
...
#print("ARGV = @ARGV<br>\n");
#print("The real path before filtering is: $path<br>\n");
...
#print("The real path after filtering is: $path<br>\n");
...
foreach $fname (@sorted) {
      ...
      #print ("<hr>Inside $path is $fname.<p>");

    $fqfname = $path . $fname;
      #print ("fqfname = :$fqfname:<br>");
...
#      print (" URL is $url.<br>");
	...
	print "<li> <a href=\"$thisscript?$url\">$fname</a>\n";
    }
}
...
foreach $fname (@sorted) {
    ...
    # Now check for directories
    if(-e $fqfname && $fname =~ /html?$/){

	# If we got here, we have at least one html. Print out our
	# header, then disable the message.

	if (!$html_seen) {
	    print ("<hr><h2>HTML:</h2> <ul>\n");
	    $html_seen = 1;
	}
