...
class QHPage
{
  ...
  function NewImageName($iImageName)
  {
    ...
    if (preg_match("?^graphix/?", $in))
    {
      $newImageName = preg_replace('?graphix/?', '/QHST/Images/graphix/', $in);
    }
    else
    {
  ...
  function PageBegin($showAds = 0)
  {
    ...
    print ('      </form>'."\n");
    print ('      <!-- SiteSearch Google -->'."\n");
    print ('    </div> <!-- End Search -->'."\n");
    print ('  </div> <!-- End HeaderRight -->'."\n");
    ...
    { // Found input file -- process it.
      ...
      foreach ($inputLines as $line)
      {
        ...
        if (preg_match("/<!DOCTYPE/i", $line) || 
            preg_match("/<html/i", $line) || 
            preg_match("?</html?i", $line) || 
            preg_match("/link.*rel.*\"shortcut icon\".*href.*=.*Master.*\.ico/i", $line) || 
